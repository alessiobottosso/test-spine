...
(function (spine) {
    var MeshAttachment = (function (_super) {
        ...
        MeshAttachment.prototype.updateUVs = function (region, uvs) {
            ...
            if (!uvs || uvs.length != n) {
                uvs = spine.Utils.newFloatArray(n);
            }
            if (region == null) {
                return;
            }
            var texture = region._texture;
            var r = texture._uvs;
...
game.Loader.inject({
    loadAtlas: function(filePath, callback) {
        this.loadFile(filePath, this.parseAtlas.bind(this, filePath, callback));
    },

    parseAtlas: function(filePath, callback, request) {
        if (!request.responseText || request.status === 404) callback('Error loading Atlas ' + filePath);
        ...
        try {
            ...
            var spineAtlas = new game.spine.TextureAtlas(request.responseText, function(image) {
                texturesToLoad++;
                return new game.spine.FakeTexture(game.BaseTexture.fromImage(game._getFilePath(image), function() {
                    texturesToLoad--;
                    if (texturesToLoad === 0) callback();
                }));
            });
            game.spine.AtlasCache[filePath] = spineAtlas;
            return;
        }

        this.parseJSON(filePath, callback, request);
    }
});
...
/**
    @class Spine
    @constructor
    @param {String} json Name of the Spine JSON file
    @param {String} [atlas] Name of the Spine Atlas file. If not defined, will use same name as for the JSON file.
    @param {Object} [props]
**/
game.createClass('Spine', 'Container', {
    ...
    **/
    skeleton: null,
    /**
        @property {spine.SkeletonData} spineData
    **/
    spineData: null,
    /**
    ...
    **/
    state: null,
    /**
        @property {spine.AnimationStateData} stateData
    **/
    stateData: null,
    /**
        @property {Array} _slotContainers
        @private
    **/
    _slotContainers: [],

    staticInit: function(json, atlas, props) {
        this.super(props);

        if (!atlas) atlas = json.replace('json', 'atlas');
        var atlasFile = game._getFilePath(atlas);
        if (game.scale > 1) {
            atlasFile = atlasFile.replace(/\.(?=[^.]*$)/, '@' + game.scale + 'x.');
        }

        var spineAtlas = game.spine.AtlasCache[atlasFile];
        if (!spineAtlas) throw 'Spine Atlas ' + atlas + ' not found';

        var jsonData = game.getJSON(json);
        if (!jsonData) throw 'Spine JSON ' + json + ' not found';

        var spineAtlasLoader = new game.spine.AtlasAttachmentLoader(spineAtlas)
        var spineJsonParser = new game.spine.SkeletonJson(spineAtlasLoader);

        this.spineData = spineJsonParser.readSkeletonData(jsonData);

        this.skeleton = new game.spine.Skeleton(this.spineData);
        this.skeleton.flipY = true;
        this.skeleton.updateWorldTransform();

        this.stateData = new game.spine.AnimationStateData(this.skeleton.data);
        this.state = new game.spine.AnimationState(this.stateData);

        for (var i = 0, n = this.skeleton.slots.length; i < n; i++) {
            var slot = this.skeleton.slots[i];
            var attachment = slot.attachment;
            var slotContainer = new game.Container();
            this._slotContainers.push(slotContainer);
            this.addChild(slotContainer);

            if (attachment instanceof game.spine.RegionAttachment) {
                var sprite = this._createSprite(slot, attachment);
                slot.currentSprite = sprite;
                slot.currentSpriteName = attachment.region.name;
                slotContainer.addChild(sprite);
            }
            else if (attachment instanceof game.spine.MeshAttachment && game.webgl) {
                var mesh = this._createMesh(slot, attachment);
                slot.currentMesh = mesh;
                slot.currentMeshName = attachment.region.name;
    ...
    /**
        Create combined skin from multiple skins.
        @method createSkin
        @param {String} name name of skin
        @param {Array} skins list of skin names
    **/
    createSkin: function(name, skins) {
        var newSkin = new spine.Skin(name);
        ...
        for (var i = 0; i < skins.length; i++) {
            ...
            if (!skin) {
                console.log('Skin not found: ' + skinName);
                continue;
            }

            for (var slotIndex in skin.attachments) {
                slotIndex = parseInt(slotIndex);
                if (!slotIndex) continue;

                var attachment = skin.attachments[slotIndex];
                var attachmentName = Object.keys(attachment)[0];
                var newAttachment = skin.attachments[slotIndex][attachmentName];

                if (!slotIndex || !attachmentName) continue;

                var oldAttachment = newSkin.getAttachment(slotIndex, attachmentName);
    ...
    /**
        ...
        @param {Boolean} [queue] Queue animation
        @param {Number} [delay] Delat of animation (seconds)
        @param {Number} [track] Animation track index
    **/
    play: function(name, loop, queue, delay, track) {
        if (!name) return;
        var func = queue ? 'addAnimation' : 'setAnimation';
        this.state[func](track || 0, name, !!loop, delay || 0);
    },
    ...
    **/
    ...
    updateTransform: function() {
        ...
        for (var i = 0, n = slots.length; i < n; i++) {
            var slot = slots[i];
            var attachment = slot.attachment;
            var slotContainer = this._slotContainers[i];
            var bone = slot.bone;
            ...
            if (attachment instanceof game.spine.RegionAttachment) {
                ...
                if (region) {
                    var spriteName = region.name;

                    if (!slot.currentSpriteName || slot.currentSpriteName !== spriteName) {
                        ...
                        if (slot.sprites[spriteName] !== undefined) {
                            ...
                            var sprite = this._createSprite(slot, attachment);
                            slot.sprites[spriteName] = sprite;
                            slotContainer.addChild(sprite);
                        }
                ...
                slotContainer._localTransform.a = bone.a;
                slotContainer._localTransform.b = bone.b;
                slotContainer._localTransform.c = bone.c;
                slotContainer._localTransform.d = bone.d;
                slotContainer._localTransform.tx = bone.worldX;
                slotContainer._localTransform.ty = bone.worldY;
                slotContainer.updateTransform = this._slotUpdateTransform;
            }
            else if (attachment instanceof game.spine.MeshAttachment && game.webgl) {
                var region = attachment.region;

                if (region) {
                    var meshName = region.name;

                    if (!slot.currentMeshName || slot.currentMeshName !== meshName) {
        ...
        for (var i = 0, n = drawOrder.length; i < n; i++) {
            var slotContainer = this._slotContainers[drawOrder[i].data.index];
    ...
    _createMesh: function(slot, attachment) {
        var region = attachment.region;
        ...
        var rotate = region.rotate ? 6 : 0;

        var texture = new game.Texture(texture._image, region.x / game.scale, region.y / game.scale, width / game.scale, height / game.scale, rotate);

        var vertices = new Float32Array(attachment.regionUVs.length);
        var uvs = new Float32Array(attachment.regionUVs.length);
        var indices = new Uint16Array(attachment.triangles);
        var mesh = new game.mesh.Mesh(texture, vertices, uvs, indices, game.mesh.Mesh.DRAW_MODES.TRIANGLES);
        mesh.alpha = attachment.color.a;
        mesh.region = region;

        region._texture = texture;
        texture._updateUvs();
        attachment.updateUVs(region, mesh.uvs);
        mesh.dirty++;

        slot.meshes = slot.meshes || {};
        slot.meshes[attachment.name] = mesh;
        return mesh;
    },

    /**
        @method _createSprite
        @private
    **/
    _createSprite: function (slot, attachment) {
        ...
        var texture = new game.Texture(texture._image, region.x / game.scale, region.y / game.scale, width / game.scale, height / game.scale);
        var sprite = new game.Sprite(texture);

        sprite.rotation = attachment.rotation * game.spine.MathUtils.degRad;
        if (region.rotate) sprite.rotation -= Math.PI / 2;
        sprite.anchorCenter();
    ...
    /**
        @method _slotUpdateTransform
        @private
    **/
    _slotUpdateTransform: function() {
